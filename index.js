...
function load_content() {
  ...
  d3.csv("text_format_data.csv").then(function(data) {
    console.log(data)
    data = data.filter(function(d) {
      return d["province_territory"] == province_code[selected_province];
    });
    data = data.map(function(d) {
      let premieres_nations = false;
      if (
        d["community_type"] == ["First Nations Community"] ||
        d["community_type"] == ["Inuit Community"]
      ) {
      ...
      return {
        ...
        csd_type: d["csd_type"],
        province_territory: d["province_territory"],
...
console.log(data)
    ...
    function right_left() {
      ...
      svg
        ...
        .text("Non-Indigenous Communities");
    ...
    function four_indexes() {
      ...
      function load_four_indexes(selected_field) {
        ...
        let indigenous_data = data.filter(function(d) {
          return (
            d["province_territory"] == province_code[selected_province] &&
            d["premieres_nations"]
          );
        });
...
function canada() {
  ...
  // Add domains
  xScale0.domain(data.map(d => d.province_territory));
...
*/
  ...
  svg
    ...
    .attr("stroke-width", 1)
    .attr("x", d => xScale0(d.province_territory))
  ...
  svg
    ...
    .attr("text-anchor", "middle")
    .attr("x", d => {
      return xScale0(d.province_territory) + xScale1.bandwidth() / 2;
    })
    .attr("y", function(d) {
  ...
  svg
    ...
    .attr("stroke-width", 1)
    .attr("x", d => xScale0(d.province_territory) + xScale1.bandwidth())
  ...
  svg
    ...
    .attr("text-anchor", "middle")
    .attr("x", d => {
      return xScale0(d.province_territory) + xScale1.bandwidth() * 1.5;
    })
    .attr("y", function(d) {
